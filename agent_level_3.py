...
def use_or_movement(world_start, world_end, inventory):
	...
	if start_view == end_view:
		...
		if object_in_front_end == object_in_front_start:
			...
			# Covering the change in inventory
			new_inventory = inventory_change(inventory, int(object_in_front_start + 0.5))
			use = object_in_front_start + 0.5
	else:
		movement = get_direction(end_state, end_view)
...
def inventory_change(inventory, action):
	if action == 3:
		...
		inventory["complex"] += num_wood + num_grass # + num_iron_stick 
		...
		#num_iron_stick = min(inventory["iron"], inventory["stick"])
		inventory["complex"] += num_wood #+ num_grass_plank + num_iron_stick
		inventory["wood"] -= num_wood
		...
		num_grass = inventory["grass"]
		num_iron_wood = min(inventory["iron"], inventory["wood"])
		#num_plank_stick = min(inventory["plank"], inventory["stick"])
		inventory["complex"] += num_grass + num_iron_wood #+ num_plank_stick
		inventory["grass"] -= num_grass
...
string_num_dict = {"w0": 3, "w1": 4, "w2": 5, "iron": 6, "grass": 7, "wood": 8}
num_string_dict = {3: "w0", 4: "w1", 5: "w2", 6: "iron", 7: "grass", 8: "wood"}
...
class Agent_Level_3():
	def __init__(self):
		...
		# Agent has a sense of direction, and a basic memory
		self.basic_inventory = {"wood": 0, "iron": 0, "grass": 0, "complex": 0}
	...
	def basic_discriminator(self, demo_model):
		...
		if not direction == None:
			...
			possibilities = []
			use, movt, _ = use_or_movement(demo_model[0], demo_model[-1], self.basic_inventory)
			if movt:
	...
	def use_object_discriminator(self, demo_model):
		...
		if end_state == penultimate_state:
			use, _, _ = use_or_movement(penultimate_world, end_world, self.basic_inventory)
			if use:
	...
	def skill_predict(self, demo):
		...
		while start_id < len(demo) - 1:
			...
			print("Segmented at: {}, skill level:{}, parameters: {}".\
				format(start_id, level_id, (num_string_dict[prediction_array[ind][level_id][0][0]], \
									prediction_array[ind][level_id][1:])))
		return params


	def describe_events(self, prev_params):
		params = []
		inventory = self.basic_inventory
