...
class CraftEnvFull(gym.Env):
    ...
    def fullstate(self, state):
        f_state = state.grid[:,:,:12]
        f_state[state.pos[0], state.pos[1], 0] = 1
        if state.dir == LEFT:
            f_state[state.pos[0], state.pos[1] - 1, 0] = -1
        elif state.dir == RIGHT:
            f_state[state.pos[0], state.pos[1] + 1, 0] = -1
        elif state.dir == UP:
            f_state[state.pos[0] - 1, state.pos[1], 0] = -1
        elif state.dir == DOWN:
            f_state[state.pos[0] + 1, state.pos[1], 0] = -1
        return f_state
    ...
    def reset(self, task = None, difficulty = 3):
        ...
        return self.fullstate(self.state)
    ...
    def step(self, action):
        ...
        return self.fullstate(s), r, s.satisfies(self.goal), {}
