...
# Make sure window width and height is odd
WINDOW_WIDTH = 5
WINDOW_HEIGHT = 5
    ...
    def visualize(self, transitions):
        def _visualize(win):
            ...
            for state, mstate in zip(states, mstates):
                ...
                print("\n\n\n\n\n\n\n\n\n\n\n\n\n\nInventory")
                print(inventory)
                #print("Wood: {}, Iron: {}".format(inventory[0]))
                #print("Iron: {}".format(inventory[1]))
                #print("Grass: {}".format(inventory[0]))
                #print("Wood: {}".format(inventory[0]))
                #print("Wood: {}".format(inventory[0]))
                #print("Wood: {}".format(inventory[0]))
                #print("Wood: {}".format(inventory[0]))
                #print("Wood: {}".format(inventory[0]))
                #print("Wood: {}".format(inventory[0]))
                #print("inventory_length: {}".format(len(inventory)))
                time.sleep(1)
                # input("?")
...
class CraftState(object):
    ...
    def satisfies(self, goal):
        return self.inventory[goal[1]] > 0

    def features(self):
        if self._cached_features is None:
            x, y = self.pos
            # Recheck if the int thing is doing any harm; what exactly are the features being computed
